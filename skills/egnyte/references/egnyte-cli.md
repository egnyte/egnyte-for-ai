...
1. **Every mutation needs `--dry-run` or `--yes`** — always run `--dry-run` first and show the user the preview
2. **Always `--fields`** on list/get calls — reduces response size and AI token cost
3. **Paths start with `/`** — never relative paths
4. **Never guess file IDs** — retrieve with `egnyte fs get` or `egnyte search` first. IDs are persistent but can become invalid if the file has been moved, deleted, or replaced since the ID was obtained.
...
# CAUTION: Unlike 'fs delete' (which moves to trash), 'trash delete' is permanent and unrecoverable. Always warn user of this distinction before proceeding.
